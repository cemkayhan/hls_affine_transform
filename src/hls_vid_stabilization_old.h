...
#pragma HLS ALLOCATION operation instances=hdiv limit=1
  ...
  std::cout << "Mat00: " << std::fixed << std::setprecision(12) << Mat[0][0] << '\n';
  std::cout << "Mat01: " << std::fixed << std::setprecision(12) << Mat[0][1] << '\n';
  std::cout << "Mat02: " << std::fixed << std::setprecision(12) << Mat[0][2] << '\n';
  std::cout << "Mat10: " << std::fixed << std::setprecision(12) << Mat[1][0] << '\n';
  std::cout << "Mat11: " << std::fixed << std::setprecision(12) << Mat[1][1] << '\n';
  std::cout << "Mat12: " << std::fixed << std::setprecision(12) << Mat[1][2] << '\n';
  //std::cout << "CenterX: " << std::fixed << std::setprecision(12) << Alpha_ << '\n';
  //std::cout << "CenterY: " << std::fixed << std::setprecision(12) << Angle*FP_T_ {CV_PI}/FP_T_ {180} << '\n';
#endif
}
...
//      ap_uint<Bit_Width<MAX_ROWS_>::Value> Mm2s_Ydec_[PPC_];
//      ap_uint<Bit_Width<MAX_ROWS_>::Value> Mm2s_Xdec_[PPC_];
//      ap_uint<Bit_Width<PPC_>::Value-2> Mm2s_Xrem_[PPC_];
//      loopCalcXYDec: for(auto J=0;J<PPC_;++J){
//#pragma HLS UNROLL
//        Mm2s_Ydec_[J]=static_cast<ap_uint<Bit_Width<MAX_ROWS_>::Value>>(Mm2s_Y_[J]);
//        Mm2s_Xdec_[J]=static_cast<ap_uint<Bit_Width<MAX_COLS_>::Value>>(Mm2s_X_[J]);
//        Mm2s_Xrem_[J]=Mm2s_Xdec_[J](Bit_Width<MAX_ROWS_>::Value-2,0);
//        Mm2s_Xdec_[J]=Mm2s_Xdec_[J](Bit_Width<MAX_ROWS_>::Value-1,Bit_Width<PPC_>::Value-1);
////#if 1!=D_PPC_
////        if(Mm2s_Xdec_[J](Bit_Width<PPC_>::Value-2,0)){
////          Mm2s_Xdec_[J]=Mm2s_Xdec_[J](Bit_Width<MAX_ROWS_>::Value-1,Bit_Width<PPC_>::Value-1)+1;
////        } else {
////          Mm2s_Xdec_[J]=Mm2s_Xdec_[J](Bit_Width<MAX_ROWS_>::Value-1,Bit_Width<PPC_>::Value-1);
////        }
////#endif
...
#if 1==D_ENABLE_C_SIMULATION_&&1==D_ENABLE_C_SIMULATION_DEBUG_
        static std::ofstream ofs1 {"frameReferenceHls2.txt"}; 
        ap_uint<2*D_DEPTH_*D_PPC_> S2mmx_=S2mm[(D_MAX_STRIDE_/D_PPC_)*Y_+X_];
        ofs1 << S2mmx_.to_string(16) << '\n';
#endif
...
#pragma HLS UNROLL
        ...
        if(Mm2s_X_[J]>=static_cast<FP_T_>(0)&&Mm2s_X_[J]<static_cast<FP_T_>(Width)&&Mm2s_Y_[J]>=static_cast<FP_T_>(0)&&Mm2s_Y_[J]<static_cast<FP_T_>(Height)){
          ...
          //const auto S2mm_ {S2mm[(MAX_STRIDE_/PPC_)*static_cast<ap_uint<Bit_Width<MAX_ROWS_>::Value>>(Mm2s_Y_[J])+static_cast<ap_uint<Bit_Width<MAX_COLS_>::Value>>(Mm2s_X_[J])]};
          //const auto S2mm_ {S2mm[(MAX_STRIDE_/PPC_)*static_cast<ap_uint<Bit_Width<MAX_ROWS_>::Value>>(Mm2s_Y_[J])+static_cast<ap_uint<Bit_Width<MAX_COLS_>::Value>>(Mm2s_X_[J])]};
          ap_uint<2*D_DEPTH_*D_PPC_> S2mm_ = S2mm[(MAX_STRIDE_/PPC_)*Mm2s_Ydec_+Mm2s_Xshift_];
          //std::cout <<PPC_ << '\n';

#if 1==D_ENABLE_C_SIMULATION_&&1==D_ENABLE_C_SIMULATION_DEBUG_
          ap_uint<16> Pix_=S2mm_(2*DEPTH_*J+2*DEPTH_-1,2*DEPTH_*J);
          ofs << "X[" << J << "]: " << std::fixed << std::setprecision(12) << Mm2s_X_[J] << ", "
              << "Y[" << J << "]: " << std::fixed << std::setprecision(12) << Mm2s_Y_[J] << '\n';
          ofs << "X[" << J << "]: " << Mm2s_Xdec_[J] << ", "
              << "Y[" << J << "]: " << Mm2s_Ydec_[J] << '\n';
          ofs << "X[" << J << "]: " << Pix_(15,8) << ", "
              << "Y[" << J << "]: " << Pix_(7,0) << '\n';
          ofs << S2mm_.to_string(16) << '\n';
#endif

#if 1==D_PPC_
          const auto Index_ {ap_uint<1> {0}};
#else
          const auto Index_ {Mm2s_Xdec_(Bit_Width<PPC_>::Value-2,0)};
#endif
          switch(Index_){
            case 0:
          ...
          Mm2s_(2*DEPTH_*J+2*DEPTH_-1,2*DEPTH_*J)=0x80;
        }
      }
